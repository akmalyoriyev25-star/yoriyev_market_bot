# -*- coding: utf-8 -*-
"""YORIYEV MARKET BOT – To'liq tuzatilgan versiya"""
...
def show_main_menu(update: Update, context: CallbackContext, edit=False):
    keyboard = [
        ...
        [InlineKeyboardButton("⚠️ Shikoyatlar", callback_data="menu_complaint")],
...
def order_name_receive(update: Update, context: CallbackContext):
    ...
    if update.message.contact:
        ...
        update.message.reply_text(
            ...
            parse_mode="HTML",
            reply_markup=ReplyKeyboardMarkup([[KeyboardButton("📱 Profilni yuborish", request_contact=True)]], 
...
def admin_contact(update: Update, context: CallbackContext):
    ...
    # Admin profiliga to'g'ridan-to'g'ri havola
    keyboard = InlineKeyboardMarkup([
...
def complaint_receive(update: Update, context: CallbackContext):
    ...
    if ADMIN_ID:
        try:
            ...
            context.bot.send_message(ADMIN_ID, admin_message, parse_mode="HTML")
            logger.info(f"Shikoyat admin ga yuborildi: {user_id}")
        except Exception as e:
            logger.error(f"Admin xabar yuborishda xatolik: {e}")
...
def cabinet(update: Update, context: CallbackContext):
    ...
    if orders:
        ...
        for o in orders:
            text += f"#{o['id']}: {o['items'][:30]}... ({o['status']})\n"
    else:
...
def admin_orders(update: Update, context: CallbackContext):
    ...
    for o in orders:
        ...
        text += (
            f"🔢 Buyurtma №{o['id']}\n"
            f"👤 Mijoz: {user['first_name'] if user else 'Noma\'lum'}\n"
            f"📞 Tel: {user['phone'] if user else 'Yo\'q'}\n"
            f"🆔 ID: {o['user_id']}\n"
            f"📦 Mahsulotlar: {o['items']}\n"
            f"📅 Sana: {o['created_at'][:16]}\n"
            f"👤 Profil: tg://user?id={o['user_id']}\n\n"
        )
...
def admin_complaints(update: Update, context: CallbackContext):
    ...
    with get_db() as conn:
        cur = conn.execute("SELECT * FROM complaints ORDER BY id DESC LIMIT 10")
    ...
    for c in complaints:
        ...
        text += (
            ...
            f"👤 Ism: {user['first_name'] if user else 'Noma\'lum'}\n"
            f"📞 Tel: {user['phone'] if user else 'Yo\'q'}\n"
            f"📝 Xabar: {c['text']}\n"
            f"📅 Sana: {c['created_at'][:16]}\n"
            f"👤 Profil: tg://user?id={c['user_id']}\n\n"
        )
